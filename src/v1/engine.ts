...
export class KeyboardEngine {
  ...
  readonly log: RawKey[] = [];
  lastErrorMs = -Infinity; // drives the error flash
  ...
  // Authoritative result: a fold over the raw keydown log, so what the report shows and what the
  // exported log recomputes to cannot disagree.
  result(): RunResult {
